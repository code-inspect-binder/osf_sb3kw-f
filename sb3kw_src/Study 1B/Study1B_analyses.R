...
setwd("X:/OneDrive/Envy/Entitlement, Status, and Envy/Schriftliches/Documentation for OSF/Study 1B")
...
# data <- import("Study1B.sav", header = TRUE)
# 
# #####
...
# 
# data <- subset(data, select = -c(V2,V3,V4,V5,V6,V7,V8,V9,consent,consentyes,pesinstr,prestdommotinstr,
#                                  envyscaleinstr,NPIinstruct,socialdesir_instr,LocationLatitude,
...
# 
# #recode unreasonable age value to the best of my knowledge
# data$age[data$age == "U24"] <- 24
# 
...
# #
# #NPI1 - "I know that I am good because everybody keeps telling me so"
...
# #Participants pick whether the first (1) or second option (2) fit better to them.
# #
# #selfesteem1 - "I have high self-esteem."
# #Response was given on a scale from 1 (strongly disagree), 2 (moderately disagree), 3 (slightly disagree),
# #4 (neither agree nor disagree), 5 (slightly agree), 6 (moderately agree), to 7 (strongly agree).
# #
...
# 
# data$age <- ifelse(data$age <= 30, 1, ifelse(data$age > 30 & data$age <= 38, 2, 3))
# 
# #age 1 'bottom third' 2 'middle third' 3 'upper third'
# 
# #Only seven participants indicated "other" for gender.
...
# 
# export(data, "Study1B.csv")
# 
# #The code below allows you to run all the analyses from the paper as long as the respective variables are unchanged.

data <- import("Study1B.csv", header = TRUE)

#####
#Demographics
#####

#One participant had missing data on all variables and was therefore excluded.

data <- data[data$id != "155", ]

describe(data.frame(data$age))
summary(data.frame(data$genderr))
...
data$NPI1r <- 2 - data$NPI1
data$NPI2r <- 2 - data$NPI2
data$NPI3r <- 2 - data$NPI3
data$NPI4r <- 2 - data$NPI4
data$NPI5r <- 2 - data$NPI5
data$NPI6r <- 2 - data$NPI6
data$NPI7r <- 2 - data$NPI7
data$NPI8r <- 2 - data$NPI8
data$NPI9r <- 2 - data$NPI9
data$NPI10r <- 2 - data$NPI10
data$NPI11r <- 2 - data$NPI11
data$NPI12r <- 2 - data$NPI12
data$NPI13r <- 2 - data$NPI13
data$NPI14r <- 2 - data$NPI14
data$NPI15r <- 2 - data$NPI15
data$NPI16r <- 2 - data$NPI16

psych::alpha(data.frame(data$NPI1r,data$NPI2r,data$NPI3r,data$NPI4r,data$NPI5r,data$NPI6r,
                        data$NPI7r,data$NPI8r,data$NPI9r,data$NPI10r,data$NPI11r,data$NPI12r,
                        data$NPI13r,data$NPI14r,data$NPI15r,data$NPI16r))

data$narcissism <- rowMeans(data[c("NPI1r","NPI2r","NPI3r","NPI4r","NPI5r","NPI6r","NPI7r",
                                   "NPI8r","NPI9r","NPI10r","NPI11r","NPI12r","NPI13r","NPI14r",
                                   "NPI15r","NPI16r")], na.rm = TRUE)
...
describe(data.frame(data$pes,data$prestige,data$dominance,data$benign,data$malicious,data$s_d,
                    data$narcissism,data$selfesteem1))

chart.Correlation(data[ , c("pes","prestige","dominance","benign","malicious","s_d",
                            "narcissism","selfesteem1")], use = "pairwise.complete.obs", pch = 20, histogram = TRUE)

#####
###Initial path model that tests the hypotheses and controls for narcissism, self-esteem, and social desirability.

model_initial_control <-
'
#regressions with constraints

prestige ~ ep*pes + np*narcissism + sep*selfesteem1 + sdp*s_d
dominance ~ ed*pes + nd*narcissism + sed*selfesteem1 + sdd*s_d

benign ~ 0*pes + pb*prestige + 0*dominance + nb*narcissism + seb*selfesteem1 + sdb*s_d
malicious ~ 0*pes + 0*prestige + dm*dominance + nm*narcissism + sem*selfesteem1 + sdm*s_d
...
pes ~~ ese*selfesteem1
pes ~~ esd*s_d
narcissism ~~ nse*selfesteem1
narcissism ~~ nsd*s_d
selfesteem1 ~~ sesd*s_d
...
fit_model_initial_control <- sem(model_initial_control, data = data, likelihood = "wishart")

summary(fit_model_initial_control, standardized = TRUE, fit.measures = TRUE, rsq = TRUE)

###We added the direct effects of entitlement on benign and malicious envy.

model_direct_control <-
'
#regressions with constraints
...
benign ~ eb*pes + pb*prestige + 0*dominance + nb*narcissism + seb*selfesteem1 + sdb*s_d
malicious ~ em*pes + 0*prestige + dm*dominance + nm*narcissism + sem*selfesteem1 + sdm*s_d
...
fit_model_direct_control <- sem(model_direct_control, data = data, likelihood = "wishart")

anova(fit_model_initial_control,fit_model_direct_control)
summary(fit_model_direct_control, standardized = TRUE, fit.measures = TRUE, rsq = TRUE)

parameterEstimates(fit_model_direct_control)

#bootstrapped bias-corrected confidence intervals for indirect effects
# fit_model_direct_control_se <- sem(model_direct_control, data = data, se = "boot", bootstrap = 5000)
# save(fit_model_direct_control_se, file = "bc_bootstrap_model_direct_control.RData")

load("bc_bootstrap_model_direct_control.RData") #loads the data from previous bootstrap
                                                #can be run again with code above; results will be slightly different
as.data.frame(parameterEstimates(fit_model_direct_control_se, boot.ci.type = "bca.simple"))[37:40, ]

#####
###Further model tests without social desirability, narcissism, and self-esteem.
...
#####
###Models including narcissism, self-esteem, and social desirability, but with narcissism being separated from its entitlement/exploitativeness facet.

data$narcissism_no_ent <- rowMeans(data[c("NPI1r","NPI2r","NPI3r","NPI4r","NPI7r","NPI8r","NPI11r",
                                          "NPI12r","NPI13r","NPI14r","NPI15r","NPI16r")], na.rm = TRUE)

model_initial_control2 <-
'
#regressions with constraints

prestige ~ ep*pes + np*narcissism_no_ent + sep*selfesteem1 + sdp*s_d
dominance ~ ed*pes + nd*narcissism_no_ent + sed*selfesteem1 + sdd*s_d

benign ~ 0*pes + pb*prestige + 0*dominance + nb*narcissism_no_ent + seb*selfesteem1 + sdb*s_d
malicious ~ 0*pes + 0*prestige + dm*dominance + nm*narcissism_no_ent + sem*selfesteem1 + sdm*s_d

#covariances

prestige ~~ pd*dominance
benign ~~ bm*malicious
pes ~~ en*narcissism_no_ent
pes ~~ ese*selfesteem1
pes ~~ esd*s_d
narcissism_no_ent ~~ nse*selfesteem1
narcissism_no_ent ~~ nsd*s_d
selfesteem1 ~~ sesd*s_d
...
fit_model_initial_control2 <- sem(model_initial_control2, data = data, likelihood = "wishart")

summary(fit_model_initial_control2, standardized = TRUE, fit.measures = TRUE, rsq = TRUE)

###We added the direct effects of entitlement on benign and malicious envy.

model_direct_control2 <-
'
#regressions with constraints
...
benign ~ eb*pes + pb*prestige + 0*dominance + nb*narcissism_no_ent + seb*selfesteem1 + sdb*s_d
malicious ~ em*pes + 0*prestige + dm*dominance + nm*narcissism_no_ent + sem*selfesteem1 + sdm*s_d
...
fit_model_direct_control2 <- sem(model_direct_control2, data = data, likelihood = "wishart")

anova(fit_model_initial_control2,fit_model_direct_control2)
summary(fit_model_direct_control2, standardized = TRUE, fit.measures = TRUE, rsq = TRUE)
parameterEstimates(fit_model_direct_control2)

#bootstrapped bias-corrected confidence intervals for indirect effects
# fit_model_direct_control2_se <- sem(model_direct_control2, data = data, se = "boot", bootstrap = 5000)
# save(fit_model_direct_control2_se, file = "bc_bootstrap_model_direct_control2.RData")

load("bc_bootstrap_model_direct_control2.RData") #loads the data from previous bootstrap
                                                 #can be run again with code above; results will be slightly different
as.data.frame(parameterEstimates(fit_model_direct_control2_se, boot.ci.type = "bca.simple"))[37:40, ]
