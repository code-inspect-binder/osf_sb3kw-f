...
setwd("X:/OneDrive/Envy/Entitlement, Status, and Envy/Schriftliches/Documentation for OSF/Study 2B")
...
# data <- import("Study2B.sav", header = TRUE)
# 
# #####
...
# 
# data <- subset(data, select = -c(V2,V7,V8,V9,consent,consentyes,mturkid,pesinstr,
...
# 
# #the first four rows were attempts by us to test the study
# 
# data <- data[data$MTurkCode != "94722599" & data$MTurkCode != "56110227" & data$MTurkCode != "35289933"
#              & data$MTurkCode != "68058163", ]
# 
...
# data$mc <- ifelse(data$gender == "female", data$mcheckfemale, data$manipcheckmale)
...
# data <- subset(data, select = -c(mcheckfemale,manipcheckmale))
# 
# #Cond == Entitlement
...
# data <- merge(data, coding, by = c("V1"), all.x = TRUE)
# 
# rm(coding)
# 
...
# 
# data$age <- ifelse(data$age <= 28, 1, ifelse(data$age > 28 & data$age <= 36, 2, 3))
# 
